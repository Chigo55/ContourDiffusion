...
class LaplacianPyramid(nn.Module):
    """
    Constructs a multi-resolution Laplacian pyramid from an image tensor.

    This module repeatedly applies Gaussian blurring and downsampling to create a
    pyramid of lower-resolution images. The difference between an image and its
    upsampled blurred version constitutes a level of the Laplacian pyramid,
    capturing details at a specific scale.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        """
        Initializes the LaplacianPyramid module.

        Args:
            in_channels (int): Number of channels in the input tensor.
            num_levels (int): The number of pyramid levels to construct.
            filter_size (int): The size of the Gaussian filter kernel (must be odd).
...
class DirectionalFilterBank(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            sigma (float): Standard deviation for the Gaussian low-pass kernel.
    ...
    def _dfb(
        ...
        """
        sheared_pos, sheared_neg = self._apply_shear(input=input)
